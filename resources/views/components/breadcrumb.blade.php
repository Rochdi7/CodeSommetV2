{{--
    ...
    Usage: @include('components.breadcrumb', ['items' => [['label' => 'Home', 'url' => route('home')], ['label' => 'Industries'], ['label' => 'E-commerce']]])
--}}
<nav class="flex items-center justify-center gap-2 text-xs text-gray-400" aria-label="Breadcrumb">
    ...
    @if(isset($item['url']) && $index < count($items) - 1) @if($item['label']==='Home' ) <a class="hover:text-gray-600 transition-colors" aria-label="Home" href="{{ $item['url'] }}">
